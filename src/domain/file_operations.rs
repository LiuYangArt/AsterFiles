...
impl OperationManager {
    ...
    pub fn submit(
        ...
        self.next_id += 1;
        self.tasks
            .insert(id, OperationTask::new(id, kind, origin_tab, items));
    ...
    pub fn active_id(&self) -> Option<OperationId> {
        self.active
    }
    pub fn has_active_tasks(&self) -> bool {
        self.tasks.values().any(|task| task.state.is_active())
    }
    pub fn start_next(&mut self) -> Result<Option<OperationId>, StateTransitionError> {
...
mod tests {
    ...
    fn manager_runs_only_one_write_task() {
        ...
        let first = manager.submit(FileOperationKind::Copy, None, vec![item("a")]);
        let second = manager.submit(FileOperationKind::Move, None, vec![item("b")]);
        assert_eq!(manager.start_next().unwrap(), Some(first));
        assert_eq!(manager.start_next().unwrap(), None);
